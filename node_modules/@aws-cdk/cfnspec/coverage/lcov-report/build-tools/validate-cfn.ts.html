...
<head>
    <title>Code coverage report for build-tools/validate-cfn.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">build-tools</a> validate-cfn.ts</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">72.41% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>84/116</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                <span class="strong">71.81% </span>
                <span class="quiet">Lines</span>
                <span class='fraction'>79/110</span>
            </div>
...
&nbsp;
  ...
  private validatePropertyType(propType: JsonValue&lt;Record&lt;string, any&gt;&gt;) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.keys(propType.value).join(',') === 'Documentation') {
...
<span class="cstat-no" title="statement not covered" >      this.validateType(propType, 'if a property type doesn\'t have "Properties", it ');</span>
    }
  }
&nbsp;
...
&nbsp;
    this.assertOptional(resType.get('Attributes'), isObject, <span class="fstat-no" title="function not covered" >(a</span>ttrs) =&gt; {
<span class="cstat-no" title="statement not covered" >      this.validateMap(attrs, <span class="fstat-no" title="function not covered" >attr </span>=&gt; {</span>
<span class="cstat-no" title="statement not covered" >        this.validateType(attr);</span>
      });
    });
  }
&nbsp;
...
&nbsp;
      this.assertOptional(prop.get('UpdateType'), <span class="fstat-no" title="function not covered" >(x</span>: any) =&gt; {
<span class="cstat-no" title="statement not covered" >        <span class="missing-if-branch" title="if path not taken" >I</span>if (!['Mutable', 'Immutable', 'Conditional'].includes(x)) {</span>
<span class="cstat-no" title="statement not covered" >          throw new Error(`invalid value for enum: '${x}'`);</span>
        }
      });
    });
  }
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (dupes.hasValue &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >!isCollectionType)</span> {
<span class="cstat-no" title="statement not covered" >      this.report(dupes, 'occurs on non-collection type');</span>
    }
  }
&nbsp;
  private assertValidPropertyTypeReference(typeName: JsonValue&lt;string&gt;) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (BUILTIN_COMPLEX_TYPES.includes(typeName.value)) {
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      this.report(x, e.message);</span>
...
function isObject(x: any): asserts x is Record&lt;string, any&gt; {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (x == null || typeof x !== 'object' || Array.isArray(x)) {
<span class="cstat-no" title="statement not covered" >    throw new Error(`expected object, found '${x}'`);</span>
  }
}
&nbsp;
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!primitives.includes(x)) {
<span class="cstat-no" title="statement not covered" >    throw new Error(`must be one of ${primitives.join(', ')}, got: ${x}`);</span>
  }
}
&nbsp;
...
&nbsp;
  public get&lt;K extends keyof A&gt;(k: K): JsonValue&lt;A[K]&gt; {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.value || typeof this.value !== 'object' || Array.isArray(this.value)) {
<span class="cstat-no" title="statement not covered" >      return new ErrorValue(`expected object, found ${this.value}`, this.path, this.value);</span>
    }
...
&nbsp;
  public <span class="fstat-no" title="function not covered" >get&lt;</span>K extends keyof A&gt;(_k: K): JsonValue&lt;A[K]&gt; {
<span class="cstat-no" title="statement not covered" >    return this as any;</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  public get v</span>alue(): A {
<span class="cstat-no" title="statement not covered" >    throw new Error(this.error);</span>
  }
}
&nbsp;
export function <span class="fstat-no" title="function not covered" >formatErrorInContext(</span>error: ValidationError) {
  let reportValue = <span class="cstat-no" title="statement not covered" >error.value.pathValue;</span>
<span class="cstat-no" title="statement not covered" >  for (let i = <span class="cstat-no" title="statement not covered" >error.value.path.length;</span> i &gt; 0; i--) {</span>
<span class="cstat-no" title="statement not covered" >    reportValue = { [error.value.path[i - 1]]: reportValue };</span>
  }
&nbsp;
  const formattedLines = <span class="cstat-no" title="statement not covered" >JSON.stringify(reportValue, undefined, 2).split('\n');</span>
&nbsp;
  const indent = <span class="cstat-no" title="statement not covered" >2 * (error.value.path.length + 1);</span>
&nbsp;
  // Insert the error message at line N with an appropriate indent
<span class="cstat-no" title="statement not covered" >  formattedLines.splice(error.value.path.length + 1, 0, `\n!!!${' '.repeat(indent - 3)}${error.message} !!!\n`);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  return formattedLines.join('\n');</span>
}
&nbsp;
async function <span class="fstat-no" title="function not covered" >main(</span>args: string[]) {
  const spec = <span class="cstat-no" title="statement not covered" >await fs.readJson(args[0]);</span>
  const errors = <span class="cstat-no" title="statement not covered" >CfnSpecValidator.validate(spec);</span>
...
<span class="cstat-no" title="statement not covered" >    for (const error of errors) {</span>
<span class="cstat-no" title="statement not covered" >      console.error(formatErrorInContext(error));</span>
    }
